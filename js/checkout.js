...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const DEFAULT_IMG =
    "https://images.unsplash.com/photo-1459411552884-841db9b3cc2a?w=200&h=250&fit=crop";
  ...
  if (promoBtn && promoInput)
    promoBtn.addEventListener("click", () => {
      ...
      const promo = window.MXStore
        ? window.MXStore.getActivePromo(code)
        : PROMO_CODES[code];
      if (promo) {
        appliedPromo = promo;
  ...
  if (placeOrderBtn)
    placeOrderBtn.addEventListener("click", async () => {
      ...
      const orderData = {
        ...
        promo: appliedPromo
          ? { code: promoInput.value.trim().toUpperCase(), ...appliedPromo }
          : null,
      ...
      try {
        ...
        console.log("Đã lưu đơn hàng lên Firebase");
      } catch (error) {
